...
### Added
- **Custom editor** (`markdown42.editor`) — replaces VS Code's default `.md` handler with a webview-based preview+edit experience. No split pane required.
- **Block-level editing** — click any paragraph, heading, list, or code block to enter inline edit mode. Textarea appears in-place with raw markdown. Blur to commit, `Escape` to cancel, `Ctrl+Enter`/`Cmd+Enter` to save immediately.
- **Incremental DOM diffing** — `renderUpdate()` compares old vs new blocks by raw content. Only changed blocks get a DOM update — unchanged blocks keep their existing elements.
...
- **Document Outline** panel in the Explorer sidebar — auto-populated from headings, clickable to scroll
- **Toggle source view** — `Ctrl+\`` (Windows/Linux) / `Cmd+\`` (macOS) switches between rendered preview and raw markdown editor
- **External link interception** — all `<a>` clicks go via `vscode.env.openExternal`. Webview never navigates itself.
- **XSS protection** — `html: false` in markdown-it strips raw HTML from untrusted `.md` files. Link hrefs validated against `http/https/mailto` allowlist before calling `openExternal`.
- **Nonce-based CSP** — fresh `crypto.randomBytes(16)` nonce per webview instance. `default-src 'none'`.
- **Undo/redo safety** — all edits go through `vscode.workspace.applyEdit` (WorkspaceEdit API), never direct buffer mutation. VS Code's undo stack stays intact.
- **Version guard** — webview messages carry a monotonic `version` counter. Stale updates from undo/redo races are discarded.
- **Settings:**
...
- Known accepted risk: `serialize-javascript` vulnerability in `mocha` devDependency — no patch available, zero runtime exposure (devDep only)

### Testing
- 72 unit tests across: `markdownParser`, `blockParser`, `documentModel`, `outlineProvider`
...
- ASCII-format tables (non-GFM) render as plain text — need GFM `|` pipe format
