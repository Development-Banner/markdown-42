...
(() => {
  ...
  var HEADING_RE = /^#{1,6} /;
  var FENCE_START_RE = /^(`{3,}|~{3,})/;
  var TABLE_ROW_RE = /^\|/;
  var LIST_RE = /^(\s*[-*+] |\s*\d+\. )/;
  var BLOCKQUOTE_RE = /^> /;
  var HR_RE = /^(-{3,}|\*{3,}|_{3,})$/;
  var SPACE_TABLE_SEP_RE = /^[-\s]+$/;
  function isSpaceAlignedTable(raw) {
  ...
  function parseBlocks(markdown) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inFence) {
        currentLines.push(line);
        if (line.trimStart().startsWith(fenceMarker) && line.trim().split("").every((c) => c === fenceMarker[0])) {
          inFence = false;
          fenceMarker = "";
        }
        continue;
      }
      if (TABLE_ROW_RE.test(line) && currentLines.length > 0 && TABLE_ROW_RE.test(currentLines[currentLines.length - 1])) {
        currentLines.push(line);
        continue;
      }
      if (line.trim() === "") {
...
`;
  ...
  function create_rule(md2, emojies, shortcuts, scanRE, replaceRE) {
    ...
    const ucm = md2.utils.lib.ucmicro;
    const has2 = md2.utils.has;
  ...
  function emoji_plugin(md2, options) {
    ...
    const opts = normalize_opts(md2.utils.assign({}, defaults, options || {}));
  ...
  function emoji_plugin2(md2, options) {
    ...
    const opts = md2.utils.assign({}, defaults, options || {});
  ...
  var md = new lib_default({
    ...
    linkify: true,
    typographer: true
  }).use(import_markdown_it_task_lists.default, { enabled: true, label: true, labelAfter: false }).use(emoji_plugin2);
  var defaultLinkOpenRenderer = md.renderer.rules["link_open"] ?? ((tokens, idx, options, _env, self) => self.renderToken(tokens, idx, options));
  ...
  function parseColumnStarts(separator) {
    const starts = [];
    let inDash = false;
  ...
  function preprocessSpaceTables(markdown) {
    const lines = markdown.split("\n");
    const output = [];
    let i = 0;
  ...
  function renderAll(markdown, container, onBlockClick) {
  ...
  function renderUpdate(newMarkdown, container, onBlockClick) {
  ...
  function createBlockElement(block2, onClick) {
    const div = document.createElement("div");
    div.className = "block";
    div.dataset["blockIndex"] = String(block2.index);
    div.dataset["startLine"] = String(block2.startLine);
  ...
  function extractHeadings(container) {
    ...
    blocks.forEach((blockEl) => {
      ...
      const hEls = blockEl.querySelectorAll(
        "h1, h2, h3, h4, h5, h6"
      );
      hEls.forEach((h, i) => {
        const level = parseInt(h.tagName[1], 10);
        const text2 = h.textContent?.trim() ?? "";
        const id = `heading-${startLine}-${i}`;
        h.id = id;
        headings.push({ level, text: text2, line: startLine, id });
      });
    });
    return headings;
  }
  ...
  // media/inlineEditor.ts
  var _activeBlockIndex = null;
  var _activeTextarea = null;
  var _activeCallbacks = null;
  function enterEditMode(blockEl, rawMarkdown, blockIndex, callbacks, renderDelay) {
    ...
    const renderedChildren = Array.from(blockEl.children);
    renderedChildren.forEach((child) => {
      child.dataset["hiddenByEditor"] = "1";
  ...
  function _commitBlock(blockEl, blockIndex, value, callbacks) {
    _cleanup(blockEl);
    callbacks.onCommit(blockIndex, value);
  }
  function _cancelBlock(blockEl, blockIndex, callbacks) {
    _cleanup(blockEl);
  ...
  function _cleanup(blockEl) {
    _activeBlockIndex = null;
    _activeCallbacks = null;
    _activeTextarea = null;
    blockEl.classList.remove("editing");
    const textarea = blockEl.querySelector("[data-editor-textarea]");
    textarea?.remove();
    Array.from(blockEl.children).forEach((child) => {
      if (child.dataset["hiddenByEditor"]) {
        child.style.display = "";
        delete child.dataset["hiddenByEditor"];
      }
    });
  }
  ...
  function _activateEditMode(blockIndex) {
    const blockEl = blocksContainer.querySelector(
  ...
  document.addEventListener("click", (e) => {
    ...
    const href = link2.dataset["href"] ?? link2.getAttribute("href");
    if (href && href !== "#") {
